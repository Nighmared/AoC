class Computer:
	...
	def do_operation(self, op_code:int, val1_indx:int, val2_indx:int, res_indx:int,verbose = False):
		if(verbose):
			print(f"got opCode {op_code}, val1 index {val1_indx}(={self.workArray[val1_indx]}), val2 index {val2_indx}(={self.workArray[val2_indx]}) and res index {res_indx} ")
		val1 =self.workArray[val1_indx]
		val2 = self.workArray[val2_indx]
		if(op_code == 1):
			self.workArray[res_indx] = val1 + val2
			if(verbose): print(f"put result of {val1} + {val2} (={val1+val2}) at index {res_indx} [check: {self.workArray[res_indx]}]")
		elif(op_code == 2):
			self.workArray[res_indx] = val1 * val2
			if(verbose): print(f"put result of {val1} * {val2} (={val1*val2}) at index {res_indx} [check: {self.workArray[res_indx]}]")
		else:
