class Computer:
    ...
    def readfile(self, filename: str):
        ...
        for line in lines:
            ...
            if instr_op in ("jmp", "nop"):
                self.possible_exchange_lines.append(
                    (self.instr_count, instr_op, instr_arg)
                )  # line number, op, arg
            self.instr_count += 1
    ...
    def work(self):
        while (
            self.instr_pointer not in self.executed_lines
            and self.instr_pointer < self.instr_count
        ):
